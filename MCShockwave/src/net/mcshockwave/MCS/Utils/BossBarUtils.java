...
public class BossBarUtils {
	...
	// Location loc) {
	// PacketPlayOutSpawnEntityLiving mobPacket = new
	...
	// }
	//
	// public static PacketPlayOutEntityMetadata getMetadataPacket(DataWatcher
	// watcher) {
	// PacketPlayOutEntityMetadata metaPacket = new
	...
	public static void displayTextBar(String text, final Player player) {
		...
		}.runTaskLater(MCShockwave.instance, 120L);
	}
		...
		new BukkitRunnable() {
			...
			public void run() {
				if ((loadUp ? health < 300 : health > 0)) {
					...
					bar.setProgress(health / 300.0);
				} else {					
					// Complete text
					bar.setTitle(completeText);
					...
					}.runTaskLater(MCShockwave.instance, 40L);

					this.cancel();
				}
			}
		}.runTaskTimer(MCShockwave.instance, delay, delay);
	}
